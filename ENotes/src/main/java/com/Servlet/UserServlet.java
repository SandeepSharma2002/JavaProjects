...
public class UserServlet extends HttpServlet {
	...
	private void doProcess(HttpServletRequest request, HttpServletResponse response) {
		...
		if(request.getRequestURI().endsWith("addnotes"))
		{
			...
			System.out.println(" Servlet "+status);
			HttpSession session;
			try {
		...
		if(request.getRequestURI().endsWith("editPost"))
		{
			int uid = Integer.parseInt(request.getParameter("uid"));
			String title = request.getParameter("title");
			String content = request.getParameter("content"); 
			System.out.println("title= " + title );
			boolean status = postService.editNote(title, content, uid);
		...
		if(request.getRequestURI().endsWith("deletePost"))
		{
			int postid = Integer.parseInt(request.getParameter("note_id"));
			boolean status = postService.deleteNote(postid);
			System.out.println(status);
			try {
				HttpSession session;
				if (status){
					session = request.getSession();
					session.setAttribute("Note-deleted","Note Deleted Successfully");
					response.sendRedirect("/ENotes/showNotes.jsp");
				} else {
					session = request.getSession();
					session.setAttribute("NotDeleted-note","Deletion Failed");
					response.sendRedirect("/ENotes/showNotes.jsp");
						}
				} catch (IOException e) {
					// TODO Auto-generated catch block
